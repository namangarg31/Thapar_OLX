...
def register(request):
    if request.method == 'POST':
        ...
        temp = email.split('@')
        if temp[1]!='thapar.edu':
            messages.error(request,'Only TIET members are allowed')
...
def confirmregister(request):
    if request.method == 'POST':
        ...
        if code == confirmcode:
            user = user.objects.create_user(username=username, email=email, password=password,  phone=phone, roll_no=roll_no ,first_name=first_name, last_name=last_name)
            user.save()
            login(request, user)
...
def send_reply(request):
    if request.method =="POST":
        ...
        send_mail(
            'Replying to your query : '+ client_message + ' By ' + lisiting + ' owner',
            message,
            'ngarg4_be19@thapar.edu',
            [email],
            fail_silently=False
